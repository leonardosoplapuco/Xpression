...
from .serializers import FavoriteMessageSerializer
...
@api_view(['GET', 'POST'])
def favorite_message_list(request, format=None):
    if request.method == 'GET':
        favorite_messages = FavoriteMessage.objects.all()
        serializer = FavoriteMessageSerializer(favorite_messages, many=True)
        return Response(serializer.data)
    elif request.method == 'POST':
        serializer = FavoriteMessageSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
@api_view(['GET', 'PUT', 'DELETE'])
def favorite_message_detail(request, id, format=None):
    try:
        favorite_message = FavoriteMessage.objects.get(pk=id)
    except FavoriteMessage.DoesNotExist:
        return Response(status=status.HTTP_404_NOT_FOUND)

    if request.method == 'GET':
        serializer = FavoriteMessageSerializer(favorite_message)
        return Response(serializer.data)
    elif request.method == 'PUT':
        serializer = FavoriteMessageSerializer(favorite_message, data=request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    elif request.method == 'DELETE':
        favorite_message.delete()
        return Response(status=status.HTTP_204_NO_CONTENT)
